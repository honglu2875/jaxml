...
def top_p_filtering(rng, logits, top_k, top_p, min_p, *args):
    ...
    #   tokens having the same logits. But the chance is arguably fairly small.
...
class SamplingMethod:
    ...
    def from_values(cls, top_k: int, top_p: float, min_p: float, temp: float):
        use_greedy = temp <= 0.0 or min_p >= 1.0 or top_p <= 0.0 or top_k <= 1
        use_top_k = top_k > 0.0 and not use_greedy
        use_top_p = top_p < 1.0 and top_p > 0.0 and not use_greedy
        use_min_p = min_p > 0.0 and min_p < 1.0 and not use_greedy
